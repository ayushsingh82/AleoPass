...
export class LeoWalletAdapter extends BaseMessageSignerWalletAdapter {
    ...
    constructor({ appName = 'sample', isMobile = false, mobileWebviewUrl} : LeoWalletAdapterConfig = {}) {
        ...
        if (this._readyState !== WalletReadyState.Unsupported) {
            ...
            // Also use polling strategy for ongoing detection
            scopePollingDetectionStrategy(() => {
                return this._checkWalletAvailability();
            });
        }
    }

    private _checkWalletAvailability(): boolean {
    ...
    async connect(decryptPermission: DecryptPermission, network: WalletAdapterNetwork, programs?: string[]): Promise<void> {
        try {
            ...
            // Re-check wallet availability when connect is called
            const wallet = window?.leoWallet || window?.leo;
            if (!wallet) {
                // Update ready state if wallet is not found
                if (this._readyState === WalletReadyState.Installed) {
                    this._readyState = WalletReadyState.NotDetected;
                    this.emit('readyStateChange', this._readyState);
                }
                throw new WalletNotReadyError('Leo Wallet not detected. Please install the Leo Wallet extension.');
            }
            
            // Update ready state if wallet is found
            if (this._readyState !== WalletReadyState.Installed) {
                this._readyState = WalletReadyState.Installed;
                this.emit('readyStateChange', this._readyState);
            }
            ...
            try {
                ...
                throw new WalletConnectionError(error?.message || 'Connection failed', error);
            }
